...
Classifies a preprocessed document image into one of CareerLens's three
supported types: resume, transcript, or certificate.

No public dataset covers all three classes together. RVL-CDIP has a
"resume" class but nothing for transcripts/certificates (this is the
dataset gap flagged in the project design notes). Strategy, consistent
with the rest of the pipeline (pretrained/fine-tuned where data exists,
prompt where it doesn't):

1. Run a pretrained RVL-CDIP document classifier as a cheap first-pass
   signal. If it confidently says "resume", trust it and skip the VLM
   call entirely (faster, no API cost).
2. Otherwise, fall back to a prompted VLM classification call — the
   only reliable way to distinguish transcript vs. certificate, since
   no pretrained classifier exists for those categories.

The RVL-CDIP model download and the VLM API call both require network
access that isn't guaranteed in every environment this module runs in.
The orchestration logic is independently testable via the `rvlcdip_fn`
/ `vlm_fn` injection points on `classify_document`, without needing
either backend to actually be reachable.
"""
